...
export const useIsEmbed = (embedSsr?: boolean) => {
  ...
  useEffect(() => {
    ...
    if (parent !== window && !_isValidNamespace) {
      log(
        "Looks like you have iframed timely but not using Embed Snippet. Directly using an iframe isn't recommended."
      );
    }
...
const methods = {
  ui: function style(uiConfig: UiConfig) {
    ...
    if (stylesConfig) {
      console.warn(
        "Timely Embed: `styles` prop is deprecated. Use `cssVarsPerTheme` instead to achieve the same effect. Here is a list of CSS variables that are supported. https://github.com/timely/timely/blob/main/packages/config/tailwind-preset.js#L19"
      );
    }
